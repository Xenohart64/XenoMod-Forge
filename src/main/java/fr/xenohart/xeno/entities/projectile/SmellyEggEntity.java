package fr.xenohart.xeno.entities.projectile;
...
public class SmellyEggEntity extends ProjectileItemEntity {
    ...
    public SmellyEggEntity(World worldIn, LivingEntity throwerIn) {
        super(ModEntityTypes.SMELLY_EGG.get(), throwerIn, worldIn);
    }

    public SmellyEggEntity(World worldIn, double x, double y, double z) {
        super(ModEntityTypes.SMELLY_EGG.get(), x, y, z, worldIn);
    }

    public SmellyEggEntity(FMLPlayMessages.SpawnEntity spawnEntity, World world) {
        this(ModEntityTypes.SMELLY_EGG.get(), world);
    }
    ...
    protected void onImpact(RayTraceResult result) {
        ...
        if (!this.world.isRemote) {
            if (this.rand.nextInt(8) == 0) {
                ...
                for (int j = 0; j < i; ++j) {
                    CluckShroomEntity chickenentity = ModEntityTypes.CLUCKSHROOM.get().create(this.world);
